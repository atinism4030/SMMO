...
import { Plus, LayoutGrid, Archive, AlertTriangle, Zap, Info, FileJson } from 'lucide-react';
...
export default function BoardsContent() {
  ...
  async function handleSave(e: React.FormEvent, forceConfirm = false) {
    e.preventDefault();
    if (!form.clientId) { toast.error(t('boards.pleaseSelectClient')); return; }
    ...
    try {
      ...
      if (!res.ok) { toast.error(data.error ?? t('boards.failedToCreateBoard')); return; }
      const count = (data.board as IBoard).autoGeneratedTasksCount ?? 0;
      toast.success(count > 0 ? t('boards.boardCreatedWithCards', { count }) : t('boards.boardCreated'));
  ...
  return (
    <>
      <Topbar title={t('boards.title')} subtitle={t('boards.subtitle')}
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            <Button variant="secondary" onClick={() => setShowImport(true)}><FileJson size={14} />{t('boards.importContentPlan')}</Button>
            <Button onClick={openForm}><Plus size={14} />{t('boards.newBoard')}</Button>
          </div>
        } />

      <div className="flex-1 overflow-y-auto p-6">
        {loading ? <LoadingSpinner fullPage /> : boards.length === 0 ? (
          <EmptyState title={t('boards.noBoardsYet')} description={t('boards.noBoardsYetDesc')} icon={LayoutGrid}
            action={<Button onClick={openForm}><Plus size={14} />{t('boards.newBoard')}</Button>} />
        ) : (
              ...
              return (
                <div key={key}>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    {isCurrentMonth && <span className="text-xs px-2 py-0.5 rounded-full bg-zinc-900 text-zinc-400">{t('boards.current')}</span>}
                    <div className="flex-1 h-px" style={{ background: 'var(--border)' }} />
                  </div>
                            ...
                            <div className="flex items-center gap-2 flex-wrap">
                              {count > 0 && (
                                <span className="text-xs flex items-center gap-1" style={{ color: 'var(--text-muted)' }}>
                                  <Zap size={11} className="text-zinc-400" />{count} {t('boards.cardsSuffix')}
                                </span>
                              )}
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={t('boards.createMonthlyBoard')} size="lg"
        footer={
            ...
            <div>
              {cardPreview > 0 && (
                <span className="text-xs flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }}>
                  ...
                  {t('boards.willGenerate', { count: cardPreview, plural: cardPreview > 1 ? 's' : '' })}
                </span>
              )}
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <Button variant="secondary" onClick={() => setShowForm(false)}>{t('boards.cancel')}</Button>
              {pendingConfirm
                ? <Button variant="danger" onClick={e => handleSave(e, true)} loading={saving}>{t('boards.createAnyway')}</Button>
                : <Button onClick={handleSave} loading={saving}>{t('boards.createBoard')}</Button>
              }
            </div>
          </div>
        }>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          {duplicateWarning && (
            <div className="flex items-start gap-3 rounded-lg border border-zinc-700 bg-zinc-900 p-3">
              ...
              <div>
                <p className="text-sm font-medium text-zinc-200">{t('boards.duplicateBoardWarning')}</p>
                <p className="text-xs mt-0.5" style={{ color: 'var(--text-secondary)' }}>{duplicateWarning} {t('boards.clickCreateAnywayToProceed')}</p>
              </div>
            </div>
          )}

          <Divider label={t('boards.basicInfo')} />

          <Select label={t('boards.client')} value={form.clientId} onChange={e => handleClientChange(e.target.value)}
            options={[{ value: '', label: t('boards.selectClient') }, ...clients.map(c => ({ value: c._id, label: c.name }))]} />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Select label={t('boards.month')} value={form.month} onChange={e => handleMonthYearChange('month', e.target.value)} options={MONTHS} />
            <Select label={t('boards.year')} value={form.year} onChange={e => handleMonthYearChange('year', e.target.value)} options={YEARS} />
          </div>
          <Input label={t('boards.boardTitle')} value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} required placeholder={t('boards.boardTitlePlaceholder')} />
          <Textarea label={t('boards.notesOptional')} value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={2} />

          <Divider label={t('boards.monthlyContentPlan')} />
          <div className="grid grid-cols-3 gap-3">
            <NumInput label={t('boards.posts')} value={plan.posts} onChange={v => setPlan(p => ({ ...p, posts: v }))} />
            <NumInput label={t('boards.reels')} value={plan.reels} onChange={v => setPlan(p => ({ ...p, reels: v }))} />
            <NumInput label={t('boards.stories')} value={plan.stories} onChange={v => setPlan(p => ({ ...p, stories: v }))} />
          </div>

          {cardPreview > 0 && (
            <>
              <Divider label={t('boards.cardSettings')} />

              <div>
                <label className="block text-xs font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>{t('boards.platforms')}</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Select label={t('boards.assignToWorker')} value={settings.defaultAssignedWorker}
                  onChange={e => setSettings(p => ({ ...p, defaultAssignedWorker: e.target.value }))}
                  options={[{ value: '', label: t('boards.leaveUnassigned') }, ...workers.map(w => ({ value: w._id, label: w.name }))]} />
                <Select label={t('boards.deadlinePattern')} value={settings.deadlinePattern}
                  onChange={e => setSettings(p => ({ ...p, deadlinePattern: e.target.value as DeadlinePattern }))}
                  options={[
                    { value: 'NONE', label: t('boards.noDeadlines') },
                    { value: 'SPREAD', label: t('boards.spreadAcrossMonth') },
                    { value: 'SAME', label: t('boards.sameDeadline') },
                  ]} />
              </div>
              ...
              {settings.deadlinePattern === 'SPREAD' && (
                <div className="flex items-start gap-2 rounded-lg border p-3" style={{ borderColor: 'var(--border)', background: 'var(--bg-elevated)' }}>
                  ...
                  <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
                    {t('boards.contentCardsSpread', { month: MONTHS[parseInt(form.month) - 1]?.label ?? '', year: form.year })}
                  </p>
                </div>
              )}

              {settings.deadlinePattern === 'SAME' && (
                <Input label={t('boards.scheduledDate')} type="date" value={settings.sameDeadline}
      ...
      <BoardImportModal open={showImport} onClose={() => setShowImport(false)} clients={clients} onImported={fetchAll} />
    </>
  );
}
