...
export default function WorkerCalendarContent() {
  ...
  useEffect(() => {
    fetch('/api/tasks?myTasks=true').then(r => r.json()).then(d => {
      const t = (d.tasks ?? []).filter((task: ITask) => task.deadline && !['DONE', 'CANCELLED', 'POSTED'].includes(task.status));
      t.sort((a: ITask, b: ITask) => new Date(a.deadline!).getTime() - new Date(b.deadline!).getTime());
      setTasks(t);
      setLoading(false);
    });
  }, []);
  ...
  const now = new Date();
  const upcoming = tasks.filter(t => t.deadline && new Date(t.deadline) >= now);
                    ...
                    return (
                      <Link key={task._id} href={`/worker/tasks/${task._id}`} className="flex items-center gap-4 p-3 rounded-lg border border-red-500/20 hover:border-red-500/40 transition-all" style={{ background: 'rgba(239,68,68,0.05)' }}>
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-red-400">{client?.name} · Was due {formatDate(task.deadline)}</p>
                        </div>
                  ...
                  {upcoming.map(task => {
                    ...
                    return (
                      <Link key={task._id} href={`/worker/tasks/${task._id}`} className="flex items-center gap-4 p-3 rounded-lg border hover:border-indigo-500/30 transition-all" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                        <div className="w-10 h-10 rounded-lg flex flex-col items-center justify-center flex-shrink-0" style={{ background: daysLeft <= 1 ? 'rgba(239,68,68,0.15)' : daysLeft <= 3 ? 'rgba(245,158,11,0.15)' : 'var(--bg-elevated)' }}>
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary)' }}>{task.title}</p>
                          <p className="text-xs" style={{ color: 'var(--text-muted)' }}>{client?.name} · {formatDate(task.deadline)}</p>
                        </div>
                        <TaskStatusBadge status={task.status} />
                      </Link>
                    );
