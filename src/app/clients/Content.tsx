...
const emptyForm = {
  ...
  status: 'ACTIVE' as ClientStatus, notes: '', driveFolderUrl: '',
};
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {clients.map((client) => (
              <div key={client._id} className="rounded-xl border p-5 hover:border-indigo-500/30 transition-all duration-150" style={{ background: 'var(--bg-card)', borderColor: 'var(--border)' }}>
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', color: 'white' }}>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editClient ? 'Edit Client' : 'New Client'} size="lg"
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <Select label="Status" value={form.status} onChange={f('status')} options={[{ value: 'ACTIVE', label: 'Active' }, { value: 'PAUSED', label: 'Paused' }, { value: 'CLOSED', label: 'Closed' }]} />
          </div>
          <div className="grid grid-cols-2 gap-4">
